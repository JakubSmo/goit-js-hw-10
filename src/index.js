...
function createCounryList(dataArray) {
  const countryList = dataArray.reduce(
    (markup, data) => markup + createCountryListItem(data),
    ''
  );
...
function createCountryCard(data) {
  ...
  const countryCard = `<img class="country-card__flag" src="${
    flags.svg
  }" alt="${flags.alt}" heigth=100%>
    ...
    <p class="country-part">Languages:<span class="country-part-info">${Object.values(
      languages
    )}</span></p>`;
